...
export const allTools: Tool[] = Object.values(TOOLS_BY_CATEGORY).flat();
...
const toolCategory: Map<string, string> = new Map(
  Object.entries(TOOLS_BY_CATEGORY).flatMap(([category, tools]) =>
    tools.map(tool => [tool.name, category] as [string, string])
  )
);
...
function enabledCategories(): Set<string> | null {
...
function activeTools(): Tool[] {
  ...
  return allTools.filter(tool => {
    ...
    if (categories && (!category || !categories.has(category))) return false;
    if (readOnly && isMutatingTool(tool.name)) return false;
    return true;
  });
}
 ...
 * all ~55 tool files.
 */
function withMcpMeta(tool: Tool) {
    ...
    inputSchema: {
      ...
      properties: {
        property_name: {
          type: 'string',
          description:
            'The Mews property (enterprise) to target. Must be exactly one of the configured property names.',
        },
        ...base.properties,
        full: {
          type: 'boolean',
          description:
            'When true, return every raw field instead of the token-efficient projection. Costly — use only when a projected field you need is missing.',
        },
      },
...
export async function executeTool(
  ...
  try {
    ...
    const propertyName =
      typeof input.property_name === 'string' ? input.property_name : undefined;
