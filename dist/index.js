...
function loadBaseConfig(configOverride) {
    const clientToken = configOverride?.clientToken || process.env.MEWS_CLIENT_TOKEN;
    if (!clientToken) {
        throw new McpError(ErrorCode.InvalidRequest, 'Invalid Mews configuration. Please set the MEWS_CLIENT_TOKEN environment variable.');
    }
    return {
        clientToken,
 ...
 */
function buildRegistry(configOverride) {
...
function convertToZodSchema(properties) {
    const zodProperties = {};
...
function initServer(configOverride) {
    return {
        smitheryServer: () => {
            ...
            for (const tool of toolDefinitions) {
                ...
                server.tool(tool.name, tool.description || "", properties, async (args) => {
                    try {
